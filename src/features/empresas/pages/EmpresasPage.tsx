    ...
    <div className="flex gap-5">
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-end justify-between mb-6 flex-wrap gap-3">
          <div>
            <h1 className="font-serif font-light text-[2.5rem] leading-none tracking-[-0.01em] text-pine-900">Empresas</h1>
            <p className="text-sm text-pine-700 mt-1">{empresas.length} empresas registradas</p>
          </div>
          <button
            onClick={() => setShowModal(true)}
            className="px-4 py-2 rounded-lg bg-brass-500 text-white text-sm font-medium hover:bg-brass-700">
            + Nueva empresa
          </button>
        </div>
        ...
        <div className="space-y-2">
          {empresas.map(emp => (
            <div
              ...
              className={`bg-white rounded-xl border shadow-sm p-4 cursor-pointer hover:border-brass-300 transition-colors ${
                selected?.id === emp.id ? "border-brass-500 ring-1 ring-brass-300" : ""
              }`}
            >
              <div className="flex items-center justify-between">
                <div>
                  <p className="font-medium text-pine-900">{emp.nombre}</p>
                ...
                <div className="flex gap-1.5 items-center">
                  ...
                  {!emp.activa && (
                    <span className="text-xs font-semibold px-2 py-0.5 rounded-full bg-khaki-200 text-pine-600">Inactiva</span>
                  )}
                </div>
              </div>
            </div>
        ...
        <div className="w-80 flex-shrink-0">
          <div className="bg-white rounded-xl border shadow-sm p-5 sticky top-4">
            <div className="flex items-start justify-between mb-4">
              <div>
                <h2 className="text-lg font-semibold text-pine-900">{empresaDetalle.nombre}</h2>
                {empresaDetalle.cif && <p className="text-xs text-pine-700">CIF: {empresaDetalle.cif}</p>}
              </div>
              <button onClick={() => setSelected(null)} className="text-khaki-400 hover:text-pine-600 text-lg leading-none">✕</button>
            </div>
            ...
            <div className="mb-4">
              <div className="flex items-center justify-between mb-2">
                <p className="text-xs font-bold uppercase tracking-widest text-pine-600">Contactos</p>
                <button
                  onClick={() => setShowContactoModal(true)}
                  className="text-xs text-brass-700 hover:text-pine-900 font-medium">
                  + Añadir
                </button>
              </div>
              {(!empresaDetalle.contactos || empresaDetalle.contactos.length === 0) && (
                <p className="text-xs text-pine-600">Sin contactos</p>
              )}
              <div className="space-y-1.5">
                {empresaDetalle.contactos?.map(c => (
                  <div key={c.id} className="bg-khaki-100 rounded-lg p-2.5">
                    <p className="text-sm font-medium text-pine-900">{c.nombre || "Sin nombre"}</p>
                    {c.cargo    && <p className="text-xs text-pine-700">{c.cargo}</p>}
                    {c.telefono && <p className="text-xs text-pine-700">{c.telefono}</p>}
                    {c.email    && <p className="text-xs text-pine-700">{c.email}</p>}
                    {c.notas    && <p className="text-xs text-pine-600 mt-1">{c.notas}</p>}
                  </div>
                ))}
              </div>
            </div>
            ...
            {/* Alumnos */}
            <div className="mb-4">
              <p className="text-xs font-bold uppercase tracking-widest text-pine-600 mb-2">
                Alumnos ({alumnosEmpresa.length})
              </p>
              {alumnosEmpresa.length === 0 && <p className="text-xs text-pine-600">Sin alumnos asignados</p>}
              <div className="divide-y divide-slate-100">
                {alumnosEmpresa.map((a: any) => (
                  <div key={a.id} className="flex items-center justify-between py-1.5">
                    <span className="text-sm text-pine-700">{a.nombre}</span>
                    <div className="flex gap-1">
                      {a.es_fundae && <span className="text-xs px-1.5 py-0.5 rounded-full bg-amber-100 text-amber-700">FUNDAE</span>}
                      {a.nivel     && <span className="text-xs text-pine-600">{a.nivel}</span>}
                    </div>
                  </div>
                ))}
              </div>
            ...
            {!confirmDelete ? (
              ...
              <div className="bg-red-50 border border-red-200 rounded-lg p-3">
                ...
                <div className="flex gap-2">
                  <button onClick={() => { setConfirmDelete(false); setDeleteError("") }} className="flex-1 text-sm py-1.5 rounded-lg bg-khaki-200 text-pine-700 hover:bg-khaki-300">
                    Cancelar
                  </button>
                  <button
        ...
        <div className="fixed inset-0 bg-black/30 flex items-center justify-center z-50" {...showModalOverlayGuard}>
          <div className="bg-white rounded-xl shadow-lg p-6 max-w-md w-full mx-4 max-h-[90vh] flex flex-col" onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-4 flex-shrink-0">
              ...
              <button onClick={() => setShowModal(false)} className="text-khaki-400 hover:text-pine-600 text-lg leading-none">✕</button>
            </div>
            <p className="text-xs text-pine-600 mb-4 flex-shrink-0">Solo el nombre es obligatorio.</p>
            <div className="overflow-y-auto flex-1 space-y-3 pr-1">
              ...
              <div>
                <label className="block text-xs font-semibold text-pine-700 mb-1">Facturación</label>
            ...
            <div className="flex gap-2 mt-4 flex-shrink-0">
              <button onClick={() => { setShowModal(false); setEmpresaError("") }} className="flex-1 px-4 py-2 rounded-lg bg-khaki-200 text-pine-700 text-sm hover:bg-khaki-300">
                Cancelar
              </button>
              <button
                onClick={() => crearEmpresaMut.mutate(form)}
                disabled={!form.nombre || crearEmpresaMut.isPending}
                className="flex-1 px-4 py-2 rounded-lg bg-brass-500 text-white text-sm hover:bg-brass-700 disabled:opacity-50">
        ...
        <div className="fixed inset-0 bg-black/30 flex items-center justify-center z-50" {...showContactoModalOverlayGuard}>
          <div className="bg-white rounded-xl shadow-lg p-6 max-w-sm w-full mx-4" onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-4">
              ...
              <button onClick={() => setShowContactoModal(false)} className="text-khaki-400 hover:text-pine-600 text-lg leading-none">✕</button>
            </div>
            ...
            <div className="flex gap-2 mt-4">
              <button onClick={() => { setShowContactoModal(false); setContactoError("") }} className="flex-1 px-4 py-2 rounded-lg bg-khaki-200 text-pine-700 text-sm hover:bg-khaki-300">
                Cancelar
              </button>
              <button
