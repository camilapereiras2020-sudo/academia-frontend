...
function ScoreBadge({ score }: { score: number }) {
  const cls = score >= 0.8
    ...
    : "bg-khaki-100 text-pine-600"
  return (
    <span className={`badge !text-[12px] !px-2 !py-0.5 normal-case ${cls}`}>
  ...
  return (
    <div>
      <div className="flex items-end justify-between mb-6 flex-wrap gap-3">
        <div>
          <h1 className="page-title">Pagos pendientes de completar</h1>
        ...
        <div className="flex gap-2 flex-wrap">
          <button onClick={() => navigate("/pagos")} className="btn-ghost">
            ← Volver a Pagos
          </button>
      ...
      {isLoading && <p className="text-ink-soft text-[15px] py-4">Cargando sugerencias...</p>}

      {!isLoading && !rows.length && (
        <div className="card !bg-white flex flex-col items-center justify-center py-16 text-ink-soft">
          <p className="text-[15px]">No hay pagos pendientes de completar.</p>
        </div>
      )}
      ...
      <div className="space-y-3">
        {rows.map(row => {
          ...
          return (
            <div key={p.id} className={`card !bg-white p-5 ${saved ? "opacity-50" : ""}`}>
              {/* Identification header — read-only, this is how you tell rows apart */}
              <div className="bg-khaki-100 border border-pine-900/10 rounded-[10px] px-4 py-3 mb-4 flex flex-wrap gap-x-6 gap-y-1.5 text-[15px]">
                <div><span className="text-pine-600">Fecha:</span> <span className="font-medium">{p.fecha}</span></div>
                <div><span className="text-pine-600">Importe:</span> <span className="font-semibold">{formatEur(Number(p.total))}</span></div>
                <div><span className="text-pine-600">Nº reservado:</span> <span className="font-mono text-[13px]">{p.numero_factura_reservado}</span></div>
              ...
              {rowError[p.id] && (
                <p className="text-red-700 text-[14px] bg-red-50 border border-red-200 px-3 py-2 rounded-[10px] mb-3">{rowError[p.id]}</p>
              )}
              ...
              <div className="mb-3 max-w-xs">
                ...
                <select disabled={saved} value={s.marca}
                  ...
                  className="input disabled:!bg-khaki-100">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                <div>
                  <label className="flex items-center gap-1.5 font-label text-[13px] font-semibold uppercase tracking-[0.08em] text-pine-700 mb-1">
                    Alumno *
              ...
              <div className="flex justify-end mt-3">
                {saved ? (
                  <span className="font-label text-[14px] font-semibold text-green-800">✓ Guardado</span>
                ) : (
                  <button
      ...
      {bulkSummary && (
        <p className="text-amber-800 text-[15px] bg-amber-50 border border-amber-200 px-4 py-3 rounded-[10px] mb-4">{bulkSummary}</p>
      )}

      {confirmBulk && (
        <div className="modal-overlay">
          <div className="bg-white rounded-2xl shadow-xl p-6 max-w-md w-full">
            <h3 className="font-head text-[22px] leading-tight text-pine-900 mb-2">Confirmar {highConfidenceReady.length} pagos</h3>
            ...
            <div className="flex justify-end gap-2">
              <button onClick={() => setConfirmBulk(false)} className="btn-ghost">
                Cancelar
              </button>
              <button onClick={handleBulkSaveHighConfidence} className="btn-primary">
                Confirmar
              </button>
            </div>
          </div>
