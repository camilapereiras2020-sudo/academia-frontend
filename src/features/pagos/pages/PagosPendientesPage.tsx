  ...
  return (
    <div>
      <div className="flex items-end justify-between mb-6 flex-wrap gap-3">
        <div>
          <h1 className="font-serif font-light text-[2.5rem] leading-none tracking-[-0.01em] text-pine-900">Pagos pendientes de completar</h1>
        ...
        <div className="flex gap-2">
          <button onClick={() => navigate("/pagos")} className="px-4 py-2 rounded-lg bg-khaki-100 text-pine-700 text-sm hover:bg-khaki-200">
            ← Volver a Pagos
          </button>
          {highConfidenceReady.length > 0 && (
            <button
              onClick={() => setConfirmBulk(true)}
              className="px-4 py-2 rounded-lg bg-brass-500 text-white text-sm hover:bg-brass-700"
            >
      ...
      {isLoading && <p className="text-khaki-400 text-sm py-4">Cargando sugerencias...</p>}
        ...
        {rows.map(row => {
          ...
          return (
            <div key={p.id} className={`bg-white rounded-xl border shadow-sm p-4 ${saved ? "opacity-50" : ""}`}>
              ...
              <div className="bg-khaki-100 border rounded-lg p-2.5 mb-3 flex flex-wrap gap-x-6 gap-y-1 text-sm">
                ...
                <div><span className="text-pine-600">Nº reservado:</span> <span className="font-mono text-xs">{p.numero_factura_reservado}</span></div>
              ...
              <div className="mb-3 max-w-xs">
                ...
                <select disabled={saved} value={s.marca}
                  ...
                  className="w-full border rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-brass-500 disabled:bg-khaki-100">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                <div>
                  <label className="flex items-center gap-1.5 text-xs font-semibold text-pine-700 mb-1">
                    Alumno *
              ...
              <div className="flex justify-end mt-3">
                {saved ? (
                  ...
                  <button
                    ...
                    className="px-3 py-1.5 rounded-lg bg-brass-500 text-white text-xs font-medium hover:bg-brass-700 disabled:opacity-50"
                  >
      ...
      {confirmBulk && (
        <div className="fixed inset-0 bg-black/30 flex items-center justify-center z-50">
          <div className="bg-white rounded-xl shadow-lg p-6 max-w-md w-full mx-4">
            <h3 className="font-semibold text-pine-900 mb-1">Confirmar {highConfidenceReady.length} pagos</h3>
            ...
            <div className="flex justify-end gap-2">
              <button onClick={() => setConfirmBulk(false)} className="px-4 py-2 rounded-lg bg-khaki-100 text-pine-700 text-sm hover:bg-khaki-200">
                Cancelar
              </button>
              <button onClick={handleBulkSaveHighConfidence} className="px-4 py-2 rounded-lg bg-brass-500 text-white text-sm hover:bg-brass-700">
                Confirmar
              </button>
            </div>
          </div>
