...
export default function PagadorDetailPage() {
  ...
  if (loadingPagador) return <p className="text-ink-soft text-[15px]">Cargando...</p>
  if (!pagador) return <p className="text-red-700 text-[15px]">Pagador no encontrado.</p>

  return (
    <div>
      <button
        onClick={() => navigate("/payers")}
        className="btn-ghost inline-flex items-center gap-1.5 mb-4"
      >
        <ArrowLeft size={15} /> Volver a Pagadores
      </button>

      <div className="flex items-start justify-between mb-6 flex-wrap gap-3">
        <div>
          <h1 className="page-title">{pagador.nombre}</h1>
      ...
      <div className="card !bg-white p-5 mb-5">
        <p className="font-label text-[14px] font-semibold uppercase tracking-[0.12em] text-brass-700 mb-3">
          {pagador.es_alumno_adulto ? "Alumno adulto" : `Alumnos (${alumnos?.length ?? 0})`}
        </p>
        {loadingAlumnos && <p className="text-ink-soft text-[15px]">Cargando...</p>}
        {!loadingAlumnos && !alumnos?.length && <p className="text-[15px] text-ink-soft">Sin alumnos vinculados.</p>}
        <div className="flex flex-wrap gap-2">
          {alumnos?.map((a) => (
            <Link
              key={a.id}
              to={`/alumnos/${a.id}`}
              className="flex items-center gap-2 min-h-[40px] bg-khaki-100 border border-pine-900/10 rounded-[10px] px-3 text-[15px] font-semibold text-ink hover:bg-khaki-200"
            >
              {a.nombre}
              <span className="text-[14px] text-ink-soft">{MARCA_LABEL[a.marca] ?? a.marca}</span>
            </Link>
          ))}
        </div>
      ...
      <div className="card !bg-white p-5 mb-5">
        <div className="flex items-center justify-between mb-1">
          <p className="font-label text-[13px] font-semibold uppercase tracking-[0.1em] text-pine-700">Facturar juntos (Bono Familia)</p>
        ...
        {loadingPagos && <p className="text-ink-soft text-[15px]">Cargando...</p>}
        {!loadingPagos && !pendientes.length && (
          <p className="text-[15px] text-ink-soft">No hay pagos pendientes de facturar para este pagador.</p>
        )}
        ...
        <div className="space-y-1.5 mb-4">
          {pendientes.map((p) => (
            <label
              key={p.id}
              className={`flex items-center gap-3 min-h-[52px] rounded-[10px] border px-3 py-2 cursor-pointer transition-colors ${
                selected.has(p.id) ? "border-brass-500 bg-khaki-100" : "border-pine-900/15 hover:bg-khaki-100"
              }`}
            >
              <input
                type="checkbox"
              ...
              <div className="flex-1 min-w-0 flex items-center justify-between gap-3 flex-wrap">
                <div>
                  <span className="text-[15px] font-semibold text-ink">{p.alumno_nombre ?? "—"}</span>
                  <span className="text-[14px] text-ink-soft ml-2">
                    {formatMonth(p.periodo)} · {MARCA_LABEL[p.marca] ?? p.marca}
                    {p.emisor_nombre && ` · ${p.emisor_nombre}`}
                  </span>
                </div>
                <div className="flex items-center gap-3">
                  <span className="text-[15px] font-semibold text-ink">{formatEur(Number(p.total))}</span>
                  <button
                    type="button"
                    onClick={(e) => { e.preventDefault(); e.stopPropagation(); setSelectedPago(p) }}
                    className="min-h-[40px] px-2 font-label text-[14px] font-semibold text-brass-700 hover:text-pine-900 flex-shrink-0"
                  >
                    Generar factura →
                  </button>
                </div>
        ...
        {seleccionados.length > 0 && (
          <div className="bg-pine-900 text-khaki-100 rounded-[12px] p-4 mb-4 flex items-center justify-between flex-wrap gap-3">
            <div>
              ...
              <p className="text-[15px] text-khaki-100/85 mt-0.5">
        ...
        {error && <p className="text-red-700 text-[15px] bg-red-50 border border-red-200 px-4 py-3 rounded-[10px] mb-4">{error}</p>}
      ...
      {yaFacturados.length > 0 && (
        <div className="card !bg-white p-5">
          ...
          <div className="space-y-1.5">
            {yaFacturados.map((p) => (
              <div key={p.id} className="flex items-center justify-between min-h-[44px] py-1.5 text-[15px] border-b border-pine-900/10 last:border-b-0">
                <span className="text-pine-700">
                  {p.alumno_nombre} · {formatMonth(p.periodo)}
                </span>
                <span className="font-mono text-[13px] text-ink-soft">{p.num_doc}</span>
              </div>
            ))}
          </div>
        </div>
