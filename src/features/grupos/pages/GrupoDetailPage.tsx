...
export default function GrupoDetailPage() {
  ...
  if (loadingGrupo) return <p className="text-ink-soft text-[15px]">Cargando...</p>
  if (!grupo) return <p className="text-ink-soft text-[15px]">Grupo no encontrado.</p>
  ...
  return (
    <div className="max-w-3xl">
      {/* Header */}
      <button onClick={() => navigate("/grupos")} className="btn-ghost mb-4">
        ← Volver a grupos
      </button>
      <div className="card !bg-white overflow-hidden mb-5">
        <div className="flex items-stretch">
          <div className="w-1.5 flex-shrink-0" style={{ background: c.accent }} />
          <div className="flex-1 p-5">
            <div className="flex items-center gap-3 flex-wrap">
              <h1 className="page-title !text-[28px]">{grupo.nombre}</h1>
              {grupo.nivel && (
            ...
            <div className="flex items-center gap-3 flex-wrap mt-2 font-label text-[15px] font-medium text-ink-soft">
              <span>👥 {grupo.alumnos_count} alumnos</span>
              {grupo.aula && <span>📍 {grupo.aula}</span>}
              {grupo.profesor_nombre && <span>🧑‍🏫 {grupo.profesor_nombre}</span>}
        ...
        <div className="border-t px-5 py-4" style={{ background: c.bg }}>
          <p className="font-label text-[14px] font-semibold uppercase tracking-[0.12em] mb-3" style={{ color: c.text }}>Horario</p>
          {horarios.length ? (
            <div className="flex flex-wrap gap-2">
              {horarios.map((h, i) => (
                <span key={i} className="inline-flex items-center min-h-[40px] font-label text-[14px] font-semibold px-4 rounded-full"
            ...
            <p className="text-[13px] opacity-70" style={{ color: c.text }}>Sin horario definido.</p>
          )}
        </div>
      </div>
      ...
      <section className="card !bg-white p-5 mb-5">
        <h2 className="font-head text-[20px] leading-tight text-pine-900 mb-3">Alumnos</h2>
        {!roster.length && <p className="text-ink-soft text-[15px]">Sin alumnos asignados.</p>}
        <div className="flex flex-wrap gap-2">
          {roster.map(a => (
      ...
      {!isReception && (
      <section className="card !bg-white p-5 mb-5">
        <h2 className="font-head text-[20px] leading-tight text-pine-900 mb-3">Registro de clases</h2>
        {sesionError && <p className="text-red-700 text-[14px] bg-red-50 border border-red-200 px-3 py-2 rounded-[10px] mb-3">{sesionError}</p>}
        <div className="flex gap-2 flex-wrap items-end mb-4">
          ...
          <div className="flex-1 min-w-[200px]">
            <label className="block font-label text-[13px] font-semibold uppercase tracking-[0.08em] text-pine-700 mb-1">Qué se cubrió</label>
        ...
        {!sesiones.length && <p className="text-ink-soft text-[15px]">Sin clases registradas.</p>}
        <div className="space-y-2">
          {sesiones.map(s => (
            <div key={s.id} className="border border-pine-900/15 rounded-[10px] px-4 py-3">
              <p className="text-[15px] font-semibold text-ink">{s.fecha}</p>
              {s.contenido ? (
                <p className="text-[15px] text-ink-soft mt-0.5">{s.contenido}</p>
              ) : (
                <p className="text-[14px] text-ink-soft italic mt-0.5">Sin contenido registrado.</p>
              )}
            </div>
          ))}
        </div>
      ...
      <section className="card !bg-white p-5 mb-5">
        <div className="flex items-center justify-between mb-3">
          <h2 className="font-head text-[20px] leading-tight text-pine-900">Tareas</h2>
          {!showTareaForm && (
            <button onClick={openTareaForm} className="min-h-[40px] px-1 font-label text-[15px] font-semibold text-brass-700 hover:text-pine-900">+ Nueva tarea</button>
          )}
        </div>

        {showTareaForm && (
          <div className="border border-pine-900/10 rounded-[12px] p-4 mb-4 bg-khaki-100">
            {tareaError && <p className="text-red-700 text-[14px] bg-red-50 border border-red-200 px-3 py-2 rounded-[10px] mb-3">{tareaError}</p>}
            <div className="grid grid-cols-2 gap-3 mb-3">
              <div className="col-span-2">
                <label className="block font-label text-[13px] font-semibold uppercase tracking-[0.08em] text-pine-700 mb-1">Título *</label>
                <input type="text" value={tareaForm.titulo}
                  onChange={e => setTareaForm(f => ({ ...f, titulo: e.target.value }))}
                  className="input" />
              </div>
              <div className="col-span-2">
                <label className="block font-label text-[13px] font-semibold uppercase tracking-[0.08em] text-pine-700 mb-1">Descripción</label>
              ...
              <div>
                <label className="block font-label text-[13px] font-semibold uppercase tracking-[0.08em] text-pine-700 mb-1">Fecha asignada</label>
                <input type="date" value={tareaForm.fecha_asignada}
                  onChange={e => setTareaForm(f => ({ ...f, fecha_asignada: e.target.value }))}
                  className="input" />
              </div>
              <div>
                <label className="block font-label text-[13px] font-semibold uppercase tracking-[0.08em] text-pine-700 mb-1">Fecha entrega *</label>
            ...
            {!roster.length && <p className="text-ink-soft text-[15px] mb-3">Sin alumnos en este grupo.</p>}
                ...
                return (
                  <div key={tc.alumno} className="flex items-center gap-3 flex-wrap">
                    ...
                    <button onClick={() => toggleTareaEstado(idx)}
                      className={`min-h-[40px] px-4 rounded-full min-w-[130px] text-center font-label text-[14px] font-semibold ${ESTADO_TAREA_COLORS[tc.estado]}`}>
        ...
        {!tareas.length && <p className="text-ink-soft text-[15px]">Sin tareas asignadas.</p>}
        <div className="space-y-2">
          {tareas.map(t => (
            <div key={t.id} className="border border-pine-900/15 rounded-[10px] px-4 py-3">
              <div className="flex items-center justify-between flex-wrap gap-2">
                <p className="text-[15px] font-semibold text-ink">{t.titulo}</p>
                <span className="font-label text-[14px] font-medium text-ink-soft">Entrega: {t.fecha_entrega}</span>
              </div>
              {t.descripcion && <p className="text-[15px] text-ink-soft mt-0.5">{t.descripcion}</p>}
              <div className="flex flex-wrap gap-2 mt-2">
                {t.completados.map(c2 => (
                  <span key={c2.id} className={`text-[13px] font-semibold px-2.5 py-1 rounded-full ${ESTADO_TAREA_COLORS[c2.estado]}`}>
      ...
      {!isReception && (
      <section className="card !bg-white p-5 mb-5">
        <h2 className="font-head text-[20px] leading-tight text-pine-900 mb-3">Seguimiento de dificultades</h2>
        {notaError && <p className="text-red-700 text-[14px] bg-red-50 border border-red-200 px-3 py-2 rounded-[10px] mb-3">{notaError}</p>}
        <div className="space-y-2 mb-4">
        ...
        {!notas.length && <p className="text-ink-soft text-[15px]">Sin dificultades registradas.</p>}
        <div className="space-y-2">
          {notas.map(n => (
            <div key={n.id} className="bg-khaki-100 rounded-[10px] p-3.5">
              <div className="flex items-center justify-between mb-0.5">
                <span className="text-[15px] font-semibold text-ink">{n.alumno_nombre} — {n.tema}</span>
                <span className="text-[14px] text-ink-soft">{n.fecha}</span>
              </div>
              <p className="text-[15px] text-ink-soft">{n.nota}</p>
            </div>
          ))}
        </div>
      </section>
      ...
      {/* Placeholders */}
      <section className="border-2 border-dashed border-pine-900/20 rounded-[12px] p-5 mb-4 text-center text-ink-soft">
        <p className="font-head text-[18px]">Materiales</p>
        <p className="text-[14px] mt-1">Próximamente: sube archivos y materiales de clase.</p>
      </section>
      <section className="border-2 border-dashed border-pine-900/20 rounded-[12px] p-5 mb-6 text-center text-ink-soft">
        <p className="font-head text-[18px]">Programa / Syllabus</p>
        <p className="text-[14px] mt-1">Próximamente: seguimiento del programa del curso.</p>
      </section>
    </div>
  )
}
