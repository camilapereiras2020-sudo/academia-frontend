    ...
    <div>
      <div className="flex items-end justify-between mb-6 flex-wrap gap-3">
        <div>
          ...
          <p className="page-subtitle">Clases programadas por semana. Reuniones y feriados, próximamente.</p>
        </div>
        <div className="flex items-center gap-2 relative">
          ...
          {showQuickAdd && (
            <>
              ...
              <div className="absolute left-0 top-[calc(100%+8px)] z-50 w-[22rem] max-w-[90vw] bg-white rounded-[12px] border border-pine-900/15 shadow-xl p-4 flex flex-col gap-2.5">
                <input
                  autoFocus
                  value={quickTitulo}
            ...
            {cells.map((cell, idx) => {
              ...
              return (
                ...
                >
                  {cell.day != null && (
                    <>
                      <span className={`text-[14px] font-semibold w-7 h-7 flex items-center justify-center rounded-full ${
                        isToday ? "bg-pine-900 text-khaki-100" : "text-pine-700"
                      }`}>
                        {cell.day}
                      </span>
        ...
        <div className="modal-overlay"
          ...
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-md flex flex-col max-h-[85vh]">
            <div className="pl-6 pr-3 py-3 border-b border-pine-900/10 flex items-center justify-between flex-shrink-0">
              ...
              <button onClick={() => openDay(null)} className="w-11 h-11 -mr-2 flex items-center justify-center rounded-[10px] text-ink-soft hover:bg-khaki-100 hover:text-pine-900 text-xl leading-none">✕</button>
            </div>
            <div className="p-6 overflow-y-auto space-y-5">
              <div className="space-y-2">
                <p className="font-label text-[13px] font-semibold uppercase tracking-[0.1em] text-pine-700">Tareas</p>
                {!selectedTareas.length && (
                  <p className="text-[15px] text-ink-soft">Sin tareas para este día.</p>
                )}
                {selectedTareas.map(t => (
                  <div key={t.id} className="flex items-center gap-2.5 min-h-[52px] px-3 py-2 rounded-[10px] border border-pine-900/15">
                    <input
                      type="checkbox"
                      checked={t.hecha}
              ...
              <div className="space-y-2">
                <p className="font-label text-[13px] font-semibold uppercase tracking-[0.1em] text-pine-700">Clases</p>
                  ...
                  selectedClases.map(({ grupo, horario }, i) => {
                    ...
                    return (
                      <button key={i} onClick={() => navigate(`/grupos/${grupo.id}`)}
                        className="w-full min-h-[56px] flex items-center gap-3 p-3 rounded-[10px] border text-left hover:bg-khaki-100 transition-colors"
